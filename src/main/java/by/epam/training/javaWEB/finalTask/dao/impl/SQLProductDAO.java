...
public class SQLProductDAO implements ProductDAO {
    //add image
    private final String INSERT_PRODUCT = "insert into products (name, description) values (?,?);";
    ...
    private final String SELECT_ID_BY_NAME = "select (idproduct) from products where name = ?;";
    ...
    public boolean addProduct(Product product) throws DAOException {
        ...
        int result = 0;
        try {
            result = executor.update(INSERT_PRODUCT, product.getName(), product.getDescription());
    ...
    public boolean deleteProduct(Product product) throws DAOException {
        ...
        int result = 0;
                try {
                    result = executor.update(DELETE_PRODUCT, product.getIdProduct());
                } catch (DAOException e) {
                    throw new DAOException(e);
                } finally {
                    executor.close();
        ...
        try{
            switch (parameter) {
                case NAME:
                    ...
                    break;
                case PRICE:
                    result = executor.update(UPDATE_PRICE, value, id);
                    break;
                case DESCRIPTION:
                    result = executor.update(UPDATE_DESCRIPTION, value, id);
                    break;
                case RATING:
                    result = executor.update(UPDATE_RATING, value, id);
                    break;
                case CURRENT_QUANTITY:
                    result = executor.update(UPDATE_QUANTITY, value, id);
                    break;
                case TOTAL_QUANTITY:
                    result = executor.update(UPDATE_TOTAL_QUANITY, value, id);
                    break;
                case IMAGE:
                    result = executor.update(UPDATE_IMAGE, value, id);
                    break;
                default: result = 0;
            }
        } catch (DAOException e) {
    ...
    public List<Product> getProductList() throws DAOException {
        ...
        try {
            ...
            productList = convertToList(resultSet);
        } catch (DAOException e) {
            throw new DAOException(e);
        } finally {
            executor.close();
    ...
    @Override
    public int getProductId(String name) throws DAOException {
        QueryExecutor executor = QueryExecutor.getInstance();
        ResultSet resultSet = executor.select(SELECT_ID_BY_NAME,name);
        int id = 0;
        try {
            if (resultSet.next()) {
